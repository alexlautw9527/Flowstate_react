  ...
  return (
    <div className="mb-5">
      <label htmlFor={htmlFor} className="font-pixel block">
        <p>{children}</p>
        <input type={inputType} id={htmlFor} name={htmlFor} className='block w-full' onChange={e => { onInputChange(e, htmlFor) }}></input>
      </label>
    </div>
  )
}
  ...
  return (
    <div className="font-pixel container">
      ...
      <div className="w-full md:w-1/3 mx-auto flex flex-col">
        <FormInput htmlFor="email" inputType="email" onInputChange={onInputChange}> EMAIL </FormInput>
        <FormInput htmlFor="password" inputType="password" onInputChange={onInputChange}> PASSWORD </FormInput>
        {isSignInSuccess === 'login fail' && <p className="mb-5">Please check.</p>}
